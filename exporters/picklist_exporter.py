...
class PicklistExporter:
    ...
    def export_picklists(self, object_names: List[str], output_path: str, 
                        export_format: str = "excel", 
                        progress_callback: Optional[Callable] = None) -> Tuple[str, Dict]:
        """
        Export picklist data for specified objects
        ...
        stats = {
            ...
            'objects_not_found_list': [], 'cancelled': False, 'api_calls_made': 0
        }
        ...
        if export_format == "csv":
            final_output_path = self.file_handler.create_csv_file(all_rows, output_path)
        else:
            final_output_path = self.file_handler.create_excel_file(all_rows, output_path)
    ...
    def _process_object(self, obj_name: str) -> ProcessingResult:
        ...
        try:
            self.sf_client.describe_object(obj_name)
        except Exception as e:
    ...
    def _query_custom_field_tooling(self, entity_def_id: str, field_name: str) -> List[PicklistValueDetail]:
        """Query using CustomField with EntityDefinition ID"""
        if self.cancel_event.is_set():
            return []
    ...
    def _query_rest_describe_for_picklist(self, object_name: str, field_name: str) -> List[PicklistValueDetail]:
        """Query using REST API describe"""
        if self.cancel_event.is_set():
            return []
        
        try:
            url = f"{self.sf_client.base_url}/services/data/v{self.sf_client.api_version}/sobjects/{object_name}/describe"
            response = self.sf_client.make_api_call(url)
    ...
    def _parse_value_set(self, metadata: dict) -> List[PicklistValueDetail]:
        ...
        try:
            ...
            for v in values:
                ...
                results.append(PicklistValueDetail(label=label, value=value_name, is_active=is_active))
