"""
Workflow Rule Detector
Detects which fields are used in workflow rules
Accuracy: 100%
"""
...
class WorkflowDetector(BaseDetector):
    """Detects workflow rule usage of fields"""
    
    def detect_usage(self, object_name: str) -> Dict[str, List[str]]:
        """
        Detect which fields are referenced in workflow rules
        
        Args:
            object_name: API name of the object
            
        Returns:
            Dictionary mapping field names to workflow rule names
        """
        # Check cache first
        ...
        self._log(f"  Detecting workflow rule usage for {object_name}...", verbose=True)
        ...
        try:
            ...
            if response and response.status_code == 200:
                ...
                self._log(f"    Found {len(rules)} workflow rules", verbose=True)
                ...
                for rule in rules:
                    ...
                    if formula:
                        ...
                        for field_name in fields_in_formula:
                            if field_name not in field_workflows:
                                field_workflows[field_name] = []
                            if rule_name not in field_workflows[field_name]:
                                field_workflows[field_name].append(rule_name)
                
                self._log(f"    ✅ Found {len(field_workflows)} fields in workflow rules", verbose=True)
        
        except Exception as e:
            self._log(f"    ❌ Workflow rule detection error: {str(e)}", verbose=True)
        
        # Cache the results
        self.set_cached_usage(object_name, field_workflows)
        return field_workflows
    
    def _extract_fields_from_formula(self, formula: str) -> List[str]:
        """
        Extract field names from workflow formula
